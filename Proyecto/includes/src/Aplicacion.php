...
class Aplicacion
{
    ...
    public function login(usuarioDAO $user)
    {
        $this->compruebaInstanciaInicializada();
        $_SESSION['username'] = $user->getUName();
        $_SESSION['tipo'] = $user->getUTipo();
        $_SESSION['DNI'] = $user-> getUDNI();
        $_SESSION['password'] = $user->getUPass();
        $_SESSION['login'] = true;
    }
    ...
    {
        $this->compruebaInstanciaInicializada();
        return $this->usuarioLogueado() && ($_SESSION['tipo'] == 1);
    }

    public function esComerciante()
    {
        $this->compruebaInstanciaInicializada();
        return $this->usuarioLogueado() && ($_SESSION['tipo'] == 2);
    }

    public function esModerador()
    {
        $this->compruebaInstanciaInicializada();
        return $this->usuarioLogueado() && ($_SESSION['tipo'] == 3);
    }
